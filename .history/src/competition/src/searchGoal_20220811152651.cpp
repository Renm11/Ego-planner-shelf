...
void chatterCallback(const geometry_msgs::PoseStamped &msg)
{
    ...
    // globaly=msg.pose.position.y;
    // globalz=msg.pose.position.z;
    ROS_INFO("I heard is globalx: %f",msg.pose.position.x);
    ROS_INFO("I heard is globaly: %f",msg.pose.position.y);
    ROS_INFO("I heard is globalz: %f",msg.pose.position.z);
    ROS_INFO("I heard is globalz: %f",globalx);
    ROS_INFO_STREAM("------------------");
}
...
int main(int argc, char **argv)
{
    ...
    // 点的坐标 
        ...
        myPoint2.pose.position.x = globalx+2;
