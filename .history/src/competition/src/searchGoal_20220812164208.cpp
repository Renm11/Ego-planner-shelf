...
int checktarget(geometry_msgs::PoseStamped &point){
    if(abs(abs(point.pose.position.x)-abs(realtime_position_x))<1 && abs(abs(point.pose.position.y)-__GXX_ABI_VERSION(realtime_position_y)<1){
        ROS_INFO_STREAM("-----dao damubiaodian----------");
        return 1;
    }
    else{
        ROS_INFO_STREAM("-----wei dao damubiaodian----------");
        ROS_INFO("I heard is z: %f",realtime_position_x);
