...
int checktarget(geometry_msgs::PoseStamped &point){
    if(abs(abs(point.pose.position.x)-abs(realtime_position_x))<0.5 && abs(abs(point.pose.position.y)-abs(realtime_position_y))<0.5){
...
int main(int argc, char **argv)
{
    ...
    //int count = 0;
    	...
    	while (ros::ok())         {   chatter_pub.publish(point);
