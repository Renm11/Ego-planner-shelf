...
void chatterCallback(const geometry_msgs::PoseStamped &msg)
{
    ...
    ROS_INFO("I heard is y: %f", realtime_position_y);
    ROS_INFO("I heard is z: %f", realtime_position_z);
    // ROS_INFO("I heard is flagTarget: %i",abs(flagTarget));
    // ROS_INFO("I heard is globalx: %f",realtime_position_x);
    // ROS_INFO_STREAM("---------------------");
}

int checktarget(geometry_msgs::PoseStamped &point_)
{
    if (abs(point_.pose.position.x - realtime_position_x) < 0.5 && abs(point_.pose.position.y - realtime_position_y) < 0.5)
    {
    ...
    {
        ROS_INFO_STREAM("-----wei dao damubiaodian----------");

         ROS_INFO("I heard is x1: %f", point_.pose.position.x);
          ROS_INFO("I heard is x2: %f", realtime_position_z);
           ROS_INFO("I heard is y1: %f", point_.pose.position.y);
            ROS_INFO("I heard is y2: %f", realtime_position_z);
        // ROS_INFO("I heard is x: %f",realtime_position_x);
