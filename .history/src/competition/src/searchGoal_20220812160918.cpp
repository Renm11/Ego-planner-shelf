...
float target_position_x=0;//目标点位置
float target_position_y=0;
float target_position_z=0;
float target_orientation_x=0;//目标点朝向
...
int checktarget(geometry_msgs::PoseStamped &point){
    if(abs(abs(point.pose.position.x)-10)<1 && abs(abs(point.pose.position.y)-10)<1)
        re
}
